...
@api_application.route("/calc/sqrt/<op>", methods=["GET"])  # Corregir la sintaxis de la ruta
def sqrt(op):
    try:
...
@api_application.route("/calc/log10/<op>", methods=["GET"])  # Corregir la sintaxis de la ruta
def log10(op):
    try:
        num = util.convert_to_number(op)
